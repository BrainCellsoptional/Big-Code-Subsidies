...
class Display():
    def __init__(self):
        ...
        #Missile images
        self.imgaim9d = [Image.open("images/aim9d.jpeg"),"aim9d"]
        self.imgaim9m = [Image.open("images/aim9m.jpeg"),"aim9m"]
        self.imgaim7e2 = [Image.open("images/aim7e2.jpeg"),"aim7e2"]
        self.imgaim7f = [Image.open("images/aim7f.jpeg"),"aim7f"]
        self.imgaim54 = [Image.open("images/aim54.jpeg"),"aim54"]
        
        self.imgr60 = [Image.open("images/r60.jpeg"),"r60"]
        self.imgr60m = [Image.open("images/r60m.jpeg"),"r60m"]
        self.imgr73 = [Image.open("images/r73.jpeg"),"r73"]
        self.imgr27t = [Image.open("images/r27t.jpeg"),"r27t"]
        self.imgr27er = [Image.open("images/r27er.jpeg"),"r27er"]

        self.missile_image_list = []
        self.missile_keys_list = []

        self.missile_info = {
            "aim9d":["AIM-9M","IRCCM",5.23,2.5,45,"9.4 kg WDU-17/B - 3.175 kg PBXN-3 filler ",85.3],
            "aim9m":["AIM-9D","IR",5,2.5,18,"11kg MK 48 2.95kg HMX ",88.5],
        ...
        if page_num <=3:
            ...
            for num in range(4):
                if num < len(selected_list[selected_aircraft]):
                    ...
                    #Button Creation  
                    missile = Button(container2,image=self.tk_image_display_missile,bd=1,relief=RAISED,bg="black",command=lambda image=self.tk_image_display_missile,missile_key=selected_list[selected_aircraft][num][1]:(self.destroy(),self.clicked(image,missile_key),self.missile_pages(selected_aircraft,air_name,page_num + 1)))
    ...
    def output_display(self,selected_aircraft,air_name,page_num):
        payload_weight = self.payload_calculator(selected_aircraft)
        self.create()
        ...
        fuel_num = IntVar()
        fuel_slider = Scale(loadout,label="Fuel Weight In Kg",variable=fuel_num,to_=0,from_=self.aircraft_stats[selected_aircraft]["fuel"],bg="black",fg="white",font=20,orient=VERTICAL,length=130,command=lambda fuel_num:self.fuel_display(fuel_num,selected_aircraft,payload_weight))
        fuel_slider.grid(row=0,column=0,sticky=SW)


        self.display = Label(loadout,wraplength=250,bg="black",fg="white",text=f"Range: 0Km\nFuel: 0Kg \nTakeoff Wight:{self.aircraft_stats[selected_aircraft]["dry"] + payload_weight}Kg",font=("Times New Roman", 20),justify=LEFT)
        self.display.grid(row=0,column=1,padx=(50,20),sticky=N)

        display2 = Label(loadout,bg="black",fg="white",text=f"Total Payload Weight: \n{payload_weight}Kg",font=("Times New Roman", 20),justify=LEFT)
        display2.grid(row=0,column=3,sticky=N)

        #Back button
        back = Button(container5,width=8,height=2,bg="black",fg="white",text="Back",font=("Times New Roman", 20),justify="center",command=lambda:(self.destroy(),self.missile_image_list.pop(),self.missile_keys_list.pop(),self.missile_pages(selected_aircraft,air_name,(page_num - 1))))
