...
class Display():
    ...
    def previos_page(self,pagenumber):
        self.destroy()
        all_pages = {
            1:self.page1,
            2:self.page2,
        }
        all_pages[(pagenumber-1)]()

    def on_hover(self,event,missille_info):
        info_list = ["when you hover","over the buttons","this text","will change"]
        self.hover_info.config(text=info_list[missille_info])

    def page1(self):
        self.create()
        title = Label(self.frame,bg="black",fg="white",text="Multi Nation Fighter jet Loadout",font=("Times New Roman", 25, "bold"),justify="center")
        title.grid(row=1,column=1,pady=(100,50),padx=(300,300))
        ...
        self.image_button1 = Button(container, image=self.tk_image1,command=lambda:(self.destroy(),self.missile_page(0,"F-14B TomCat")))
        ...
        self.image_button2 = Button(container, image=self.tk_image2,command=lambda:(self.destroy(),self.missile_page(1,"F-4E Phantom")))
        ...
        self.image_button3 = Button(container, image=self.tk_image3,command=lambda:(self.destroy(),self.missile_page(2,"MIG-23ML Flogger")))
        ...
        self.image_button4 = Button(container, image=self.tk_image4,command=lambda:(self.destroy(),self.missile_page(3,"SU-27 Flanker")))
        ...
        f4_E = Label(container,fg="white",bg="black",text="F4-E",font=("Times New Roman", 15),justify="center")
    ...
    def missile_page(self,selected_aircraft,air_name):
        self.page_num = self.page
        for page in range(3):
            self.create()
            title_missile = Label(self.frame,bg="black",fg="white",text="Outer Wing Pillon Selection",font=("Times New Roman", 25, "bold"),justify="center")
            ...
            #----------------------------------------
            imgf14 = Image.open("images/F-14B.jpeg")
            imgf4 = Image.open("images/F-4E.jpeg")
            imgmig = Image.open("images/MIG-23ML.jpeg")
            imgsu = Image.open("images/SU-27.jpeg")
            imglist = [imgf14,imgf4,imgmig,imgsu]

            self.img_display = imglist[selected_aircraft].resize((500,300))
            self.tk_image_display = ImageTk.PhotoImage(self.img_display)

            img_box = Label(self.frame,image=self.tk_image_display)
            ...
            img_f_14b = [self.imgaim9d,self.imgaim9m]
            img_f_fe = [self.imgaim9d]
            img_mig_23ml = [self.imgr27t]
            img_su_27 = [self.imgr60,self.imgr60m,self.imgr73]
            selected_list = [img_f_14b,img_f_fe,img_mig_23ml,img_su_27]
            ...
            for num in range(4):
                if num < len(selected_list[selected_aircraft]):
                    ...
                    missile = Button(container2,image=self.tk_image_display_missile,bg="black",fg="white",text="clickme")
                    missile.grid(row=row[num],column=col[num],padx=10,pady=5)

                    missile.bind("<Enter>", lambda event ,button_num=num: self.on_hover(event,button_num))

                    missile.image = self.tk_image_display_missile
                else:
                    plac_holder = Label(container2,bg="black",width=30,height=10)
                    plac_holder.grid(row=row[num],column=col[num],padx=10,pady=5)
            ...
            back = Button(self.frame,bg="black",fg="white",text="Back",font=("Times New Roman", 20),justify="center",command=lambda:self.previos_page(page_num))
